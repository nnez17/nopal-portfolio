  ...
  return (
    <section id="projects" className="relative overflow-hidden py-32">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
          ...
          <div className="space-y-16">
            {repos.map((project, idx) => (
              <SectionReveal key={project.id} delay={idx * 0.1}>
                <div className="group relative border-t border-(--color-border) pt-8">
                  <div className="grid gap-6 md:grid-cols-3 md:gap-12">
                    <div className="md:col-span-1">
                      <span className="text-xs text-(--color-text-muted)">
                        {String(idx + 1).padStart(2, "0")}
                      </span>
                      <h3 className="mt-2 text-2xl font-semibold tracking-tight text-(--color-foreground) transition-colors group-hover:text-(--color-accent-blue)">
                        {project.name}
                      </h3>
                    </div>
                    <div className="md:col-span-2">
                      ...
                      <div className="flex items-center gap-4">
                        <a
                          ...
                          className="flex items-center gap-2 text-sm text-(--color-text-secondary) transition-colors hover:text-(--color-foreground)"
                        >
                          <Github size={16} />
                          Source
                        </a>
        ...
        {repos.length === 0 && (
          <SectionReveal>
            <div className="border-t border-(--color-border) pt-16 text-center">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-4 inline-flex items-center gap-2 text-sm text-(--color-accent-blue) hover:underline"
              >
                <Github size={16} />
        ...
        <SectionReveal delay={0.3}>
          <div className="mt-20 text-center">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-2 border-b border-(--color-border) pb-1 text-sm text-(--color-text-secondary) transition-colors hover:border-(--color-foreground) hover:text-(--color-foreground)"
            >
              View All Projects
              <ExternalLink
                size={14}
