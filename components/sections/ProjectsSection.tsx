      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        <div className="project-grid-header mb-16 text-center">
          ...
          <p className="mx-auto max-w-2xl text-slate-400">
            ...
            sorted by recent activity — no backend database needed.
          </p>
        </div>
            ...
            return (
              <div key={project.id} className="group relative">
                ...
                <article className="project-card-animate relative flex h-full flex-col rounded-2xl acrylic-strong p-6 transition-all duration-300 group-hover:-translate-y-2 group-hover:border-blue-500/40">
                  ...
                  <h3 className="mb-3 grow-0 text-xl font-semibold text-white transition-colors group-hover:text-cyan-300">
                    {project.name}
                  </h3>
                  ...
                  <div className="mt-auto flex flex-wrap gap-2">
                    {displayTags.map((tag) => (
                      <span
                        key={`${project.id}-${tag}`}
                        className="rounded-full acrylic-light px-3 py-1 text-xs font-medium text-cyan-400"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  {(project.stars > 0 || project.forks > 0) && (
                    <div className="mt-4 flex gap-4 border-t border-slate-700/40 pt-3 text-xs text-slate-500">
                      {project.stars > 0 && <span>★ {project.stars}</span>}
                      {project.forks > 0 && <span>Forks {project.forks}</span>}
                    </div>
                  )}
                </article>
              </div>
        ...
        {repos.length === 0 && (
          <div className="mx-auto mt-10 max-w-lg rounded-2xl acrylic-strong p-10 text-center text-slate-400">
            <p className="mb-6">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-full acrylic px-6 py-3 font-medium text-white transition-colors hover:bg-white/10"
            >
        ...
        <div className="mt-12 text-center">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="group inline-flex items-center gap-2 rounded-full acrylic px-8 py-3.5 font-medium text-white transition-all duration-300 hover:scale-105 hover:bg-white/10"
          >
            <Github size={20} />
            View All on GitHub
            <ExternalLink
